...
//
//clock_seconds += 1; // this could be a variable for seconds etc.
//
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE END WHILE */
	  ...
	  //transmit this char sequence to clear the putty terminal screen
	  sprintf(txd_msg_buffer, "\e[1;1H\e[2J");
	  HAL_UART_Transmit(&huart6,txd_msg_buffer, strlen((char*)txd_msg_buffer), 1000);

	  // transmit updated data
	  sprintf(txd_msg_buffer, "h:%d m:%d s:%d\n",hours,minutes,seconds);
	  HAL_UART_Transmit(&huart6,txd_msg_buffer, strlen((char*)txd_msg_buffer), 1000);
	  HAL_Delay(1000);
    /* USER CODE BEGIN 3 */
  }
